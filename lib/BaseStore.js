...
class BaseStore extends _BaseClass.default {
  ...
  get defaults() {
    return {
      ...
      proxy: _BaseProxy.default.globalDefaultProxyConfig(),
      isPaginated: false,
      fetchUrl: null,
      pageSize: 20,
      autoSort: false,
  ...
  get proxy() {
    if (!this._innerProxy) {
      this._innerProxy = _BaseClass.default.createInstance(this.defaults.proxy);
    }
    return this._innerProxy;
  }
  set proxy(config) {
    this._innerProxy = _BaseClass.default.createInstance(config);
  }
