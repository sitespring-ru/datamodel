...
class BaseStore extends _BaseClass.default {
  ...
  get model() {
    return this.initialConfig.model || _BaseModel.default;
  }
   ...
   * */
  get proxyConfig() {
    return this.initialConfig.proxy || _BaseProxy.default;
  }
  /**
  ...
  configureProxy(config) {
    if (config instanceof _BaseProxy.default) {
      return config;
    }
    ...
    if (typeof config === 'object') {
      const ct = config.type || _BaseProxy.default;
      (0, _lodashEs.unset)(config, 'type');
  ...
  loadModels(modelsOrAttrs, options = {}) {
    ...
    (0, _lodashEs.each)(modelsOrAttrs, def => {
      const {
        model,
        isCreated
      } = this.__internalAdd(def, options);
  ...
  __internalAdd(modelOrAttrs, options = {}) {
    ...
    if (!(modelOrAttrs instanceof this.model)) {
      model = new this.model(modelOrAttrs, {
        proxy: this.proxy.constructor,
        // Model in store has the same proxy type
  ...
  async fetch(config = {}) {
    ...
    const requestConfig = (0, _lodashEs.merge)({}, {
      url,
      params
    }, {
      ...config
  ...
  async search(value) {
    ...
    if ((0, _lodashEs.isEmpty)(trimmedValue)) {
      return this.fetch();
    }
    return this.fetch({
      params: {
  ...
  _serializeFiltersToRequestParams() {
    ...
    if (this.hasFilters) {
      Object.assign(filters, {
        [this.initialConfig.filterParam]: JSON.stringify((0, _lodashEs.values)(this._innerFilters))
      });
    }
    return filters;
  }
  ...
  _parsePaginationFromResponse(response) {
    let metas = (0, _lodashEs.get)(response, '_meta');
    if (!metas) {
      return null;
    }
    return {
