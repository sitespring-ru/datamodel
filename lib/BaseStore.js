...
class BaseStore extends _BaseClass.default {
  ...
  __internalAdd(modelOrAttrs) {
    ...
    if (!(modelOrAttrs instanceof _BaseModel.default)) {
      model = _BaseModel.default.createInstance(this.getModelConfig());
  ...
  remove(model) {
    return Boolean(this.removeBy(m => (0, _lodash.isEqual)(m.getId(), model.getId())));
  }
  /**
  ...
  removeBy(predicate) {
    const removeModels = (0, _lodash.remove)(this._innerModels, predicate);
  ...
  getProxy() {
    if (!this._innerProxy) {
      ...
      this._innerProxy = _BaseClass.default.createInstance(this.getProxyConfig());
    }
