...
class BaseProxy extends _BaseClass.default {
   ...
   * */
  createAxios(config = {}) {
    if (config instanceof _axios.default) {
      return config;
    }
  ...
  parseResponseData(axiosResponseSchema) {
    return (0, _lodashEs.get)(axiosResponseSchema, this.envelopeName, null);
  }
  ...
  handleResponseError(axiosErrorSchema) {
    ...
    if (axiosErrorSchema.response) {
      ...
      this.errorMessage = (0, _lodashEs.get)(axiosErrorSchema, 'response.data.message') || (0, _lodashEs.get)(axiosErrorSchema, 'response.message') || defaultErrorMessage;
  ...
  get requestHeaders() {
    ...
    if (this.constructor.hasBearerToken) {
      (0, _lodashEs.set)(headers, 'Authorization', `Bearer ${this.constructor.bearerToken}`);
    }
    return headers;
  }
  get requestParams() {
  ...
  async doRequest(extraConfig = {}) {
    ...
    const config = (0, _lodashEs.merge)({}, !(0, _lodashEs.isEmpty)(headers) ? {
      headers
    } : {}, !(0, _lodashEs.isEmpty)(params) ? {
      params
    } : {}, extraConfig);
