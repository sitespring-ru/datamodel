...
class BaseModel extends _BaseClass.default {
  ...
  constructor($attributes = {}, $config = {}) {
    ...
    this._relations = {}; // Задаем начальные данные
  ...
  __createMagicProps() {
    (0, _lodash.forEach)(this.fields(), (value, attrName) => {
  ...
  __createRelation(name) {
    ...
    if (type === 'hasMany') {
      Object.defineProperty(this, name, {
        get() {
          if (!this._relations[name]) {
            const store = _BaseStore.default.createInstance({
              model: modelConstructor,
              filters: {
                id: {
                  property: foreignKey,
        ...
        set(store) {
          if (!(store instanceof _BaseStore.default)) {
  ...
  __fetchRelationsData(data) {
    (0, _lodash.forEach)(data, (value, attr) => {
