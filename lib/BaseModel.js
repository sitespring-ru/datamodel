...
class BaseModel extends _BaseClass.default {
  ...
  applyFilter($filterDef, $rawValue, $args = []) {
    ...
    if ((0, _lodash.isArray)($filterDef)) {
  ...
  constructor($attributes = {}, $config = {}) {
    super($config); // Задаем начальные данные

    _defineProperty(this, "entityName", 'base-model');

    _defineProperty(this, "_savedAttributes", {});

    _defineProperty(this, "_dirtyAttributes", {});

    _defineProperty(this, "_isPhantom", true);
  ...
  isDirty() {
    return !(0, _lodash.isEmpty)(this._dirtyAttributes);
  }
  /**
  ...
  getSubmitValues($names = null) {
    ...
    return (0, _lodash.mapValues)($attrs, ($value, $attr) => this.applyFilter($filters[$attr], $value));
  }
  /**
   * Сохраненные данные
  ...
  getSavedAttributes($names) {
    return $names ? (0, _lodash.pick)(this._savedAttributes, $names) : this._savedAttributes;
  }
  /**
  ...
  getSavedAttribute($name) {
    if (!(0, _lodash.has)(this._savedAttributes, $name)) {
  ...
  getAttribute($name, $isDirty = true) {
    const $attrs = $isDirty ? { ...this._savedAttributes,
      ...this._dirtyAttributes
    } : this._savedAttributes;

    if (!(0, _lodash.has)($attrs, $name)) {
      throw new Error(`Unknown attribute name "${$name}"`);
    }
  ...
  getAttributes($names = null, $isDirty = true) {
    ...
    return $names ? (0, _lodash.pick)($attrs, $names) : $attrs;
  }
  /**
  ...
  _innerSetAttributes($attrs) {
    ...
    const $pickedAttrs = (0, _lodash.pick)($attrs, (0, _lodash.keys)(this._savedAttributes));
    let $changedAttrs = {};
    (0, _lodash.each)($pickedAttrs, ($value, $attr) => {
      const $filteredValue = this.applyFilter($filters[$attr], $value);

      if (!(0, _lodash.isEqual)($filteredValue, this._savedAttributes[$attr])) {
  ...
  setAttributes($attrs) {
    ...
    if (!(0, _lodash.isEmpty)($changedAttrs)) {
  ...
  validate($names = null, $extraConfig = null) {
    ...
    const $constraintsToValidate = $names ? (0, _lodash.pick)($rules, $names) : $rules; // Поскольку validate.js не поддерживает создание экземпляра
    // Возможно передать дополнительные опции при валидации, которые мы берем из конфигурации Модели

    const $options = { ...this.validateConfig,
      ...$extraConfig
    };
    const $attrs = this.getAttributes($names);
  ...
  hasErrors() {
    return !(0, _lodash.isEmpty)(this._errors);
  }
  /**
   * Сброс ошибок
   * */
  ...
  getFirstErrorMessage() {
    return (0, _lodash.get)((0, _lodash.values)(this._errors), '[0][0]', null);
  }
  /**
  ...
  getProxy() {
    if (!this.__innerProxy) {
      ...
      this.__innerProxy = _BaseClass.default.createInstance(this.proxyConfig);
    }
  ...
  _parseResponseValidationErrors($responseData) {
    ...
    * [
    ...
    if ($responseData[0] && $responseData[0]['message']) {
      ...
      return (0, _lodash.reduce)($responseData, (result, value) => {
  ...
  async save() {
    ...
    const data = this.getSubmitValues((0, _lodash.keys)(this._dirtyAttributes));
...
_defineProperty(BaseModel, "EVENT_ATTRIBUTES_CHANGE", 'attributesChanged');

_defineProperty(BaseModel, "EVENT_FETCH", 'fetch');

_defineProperty(BaseModel, "EVENT_CREATE", 'create');

_defineProperty(BaseModel, "EVENT_SAVE", 'save');

_defineProperty(BaseModel, "EVENT_DELETE", 'delete');