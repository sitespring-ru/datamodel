...
class BaseClass {
   ...
   * */
  constructor(config = {}) {
    this.initialConfig = (0, _lodashEs.defaultsDeep)(config || {}, this.defaults);
  }
  ...
  on(event, handler) {
    ...
    if ((0, _lodashEs.isArray)(event)) {
      (0, _lodashEs.forEach)(event, eventItem => this.emitter.on(eventItem, handler));
    } else {
  ...
  off(event, handler) {
    ...
    if ((0, _lodashEs.isArray)(event)) {
      (0, _lodashEs.forEach)(event, eventItem => this.emitter.off(eventItem, handler));
    } else {
